...
import '../../../services/kiosqueService.dart';
import '../../../util/Colores.dart';
import 'package:http/http.dart' as http;
import 'dart:convert';
import 'package:untitled/util/serveur.dart';

class  KiosquePg extends StatefulWidget {


  @override
  _KiosquePgState createState() => _KiosquePgState();
}

class _KiosquePgState extends State<KiosquePg> {

  late Future<List<Kiosque>> kiosques;
  ...
  Future<List<Kiosque>> getData() async {
    String url = 'http://${localhost.server}:3000/kiosque/getall';

    var jsonData = await http.get(Uri.parse(url));

    if (jsonData.statusCode == 200) {
      List data = jsonDecode(jsonData.body);
      List<Kiosque> allKiosques = [];

      for (var kiosqueJson in data) {
        Kiosque kiosque = Kiosque.fromJson(kiosqueJson);
        allKiosques.add(kiosque);
      }
  ...
  Future<void> updateRating(String hospitalName, double newRating) async {
    ...
    final response = await http.patch(
        Uri.parse('http:/${localhost.server}:3000/kiosque/update/$hospitalName'),
        body: {'rating': newRating});
    if (response.statusCode != 200) {
      throw Exception('Failed to update rating');
    }

    final responseAvg = await http.get(Uri.parse('http://${localhost.server}:3000/kiosque/getall'));
  ...
  List<Kiosque > getFilteredHospitals(List<Kiosque > kiosques) {
        ...
        title:Container(
          ...
          child:TextField(
            decoration: InputDecoration(
              hintText: 'chercher les kiosques par nom',
      ...
      body: _loading
        ...
        child: Column(
          children: [
            ...
            const SizedBox(height: 10),
              FutureBuilder<List<Kiosque>>(
                  future: kiosques, // this is a Future<List<Kiosque>>
                  builder: (BuildContext context, AsyncSnapshot<List<Kiosque>> snapshot) {
